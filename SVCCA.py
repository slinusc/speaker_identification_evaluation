...
class SVCCA:
    ...
    def compute_cca_gpu(self, X, Y, num_components):
        ...
        X /= torch.norm(X, dim=0, keepdim=True) + 1e-10
        Y /= torch.norm(Y, dim=0, keepdim=True) + 1e-10
        ...
        # Compute the inverses of covariance matrices with SVD for stability
        U_XX, S_XX, Vt_XX = torch_svd(C_XX + reg * torch.eye(C_XX.size(0), device=self.device))
        inv_S_XX = torch.diag(1.0 / S_XX)
        inv_C_XX = torch.matmul(Vt_XX.T, torch.matmul(inv_S_XX, U_XX.T))

        U_YY, S_YY, Vt_YY = torch_svd(C_YY + reg * torch.eye(C_YY.size(0), device=self.device))
        inv_S_YY = torch.diag(1.0 / S_YY)
        inv_C_YY = torch.matmul(Vt_YY.T, torch.matmul(inv_S_YY, U_YY.T))
    ...
    def perform_svcca(self, hidden_states, labels):
        ...
        hidden_states_normalized = scaler.fit_transform(hidden_states).astype(np.float64)
        labels_normalized = scaler.fit_transform(labels).astype(np.float64)  # Change to float64 for consistency
